...
class Network():
    ...
    def set_crs(self, crs=None, epsg=None):
        ...
        self.edges = pygeos.geometry.set_srid(pygeos.point, epsg)
        self.nodes = pygeos.geometry.set_srid(pygeos.point, epsg)
    ...
    for node in tqdm(network.nodes.itertuples(index=False), desc="link", total=len(network.nodes)):
        ...
        for edge in edges.itertuples():
            ...
            if point != node.geometry:
                ...
                # add edges linking
                line = pygeos.LineString([node.geometry, point])
...
def clean_roundabouts(network):
    ...
    for roundabout in roundabouts:
        ...
        for e in edges_intersect.items():
            edge = edges.iloc[e[0]]
            start = pygeos.get_point(e[1],0)
            end = pygeos.get_point(e[1],-1)
            ...
            if edge.osm_id in new_edge_id:
                ...
                start = pygeos.get_point(double_edge[-1],0)
                end = pygeos.get_point(double_edge[-1],-1)
                first_co_is_closer = pygeos.measurement.distance(end, round_centroid) > pygeos.measurement.distance(start, round_centroid) 
...
def add_distances(network):
    """This method adds a distance column using pygeos (converted from pygeos) 
    ...
    lat = pygeos.get_y(network.nodes['geometry'].iloc[0])
    lon = pygeos.get_x(network.nodes['geometry'].iloc[0])
...
def merge_edges(network, print_err=False):
    ...
    while n2:   
        ...
        if pygeos.get_type_id(new_merged_geom) == 1: 
            edg.at[info_first_edge,'geometry'] = new_merged_geom
            if nodGeom[nextNode1]==pygeos.get_point(new_merged_geom,0):
            ...
            if print_err: print("Line", info_first_edge, "failed to merge, has pygeos type ", pygeos.get_type_id(edg.at[info_first_edge,'geometry']))
...
def d_within(geom, df, distance):
    """Find the subset of a DataFrame within some distance of a pygeos geometry
...
def intersects(geom, df, sindex, tolerance=1e-9):
    """Find the subset of a GeoDataFrame intersecting with a pygeos geometry
...
def line_endpoints(line):
    ...
    Returns:
        [type]: [description]
    """    
    start = pygeos.get_point(line,0)
    end = pygeos.get_point(line,-1)
    return start, end
